...
def calculate_precision(csv_file):
    ...
    if not all(col in df.columns for col in ["Latitud", "Longitud", "Altitud(m)"]):
        raise ValueError(
            "El archivo CSV debe contener las columnas 'Latitud', 'Longitud', 'Altitud(m)'"
        )
    ...
    for index, row in df.iterrows():
        ...
        if altitud_real == 0:
            raise ValueError(
                f"Altitud real es cero en la fila {index}. No se puede calcular el error porcentual."
            )
...
# Ejemplo de uso
csv_file = os.path.join(PROJECT_ROOT, "data", "linea_d2.csv")
